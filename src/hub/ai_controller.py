...
→ Check for `washing_machine_192_168_0_5.py` tool
→ If not found, generate from SmartThings API docs
→ Prompt for SmartThings PAT token if needed
→ Execute start_wash_cycle() command

**"Check temperature from sensor at 192.168.1.50"**  
→ Look for sensor communication tool
→ Generate from Modbus/TCP docs if needed
→ Connect via TCP socket and read temperature register

**"Discover all devices on network"**
→ Scan IP range from config (192.168.1.0/24)
→ Identify device types and update device registry
